...
<body>
    ...
    <!--作品表示 -->
    <a href='../user/user_top.php'>管理画面トップへ</a><br>
    <table border="1">
        <tr>
            <td>作品名</td>
            <td>作品</td>
        ...
        <?php foreach($conts_data as $val):?>
            <tr>
                ...
                <td><a href="cont_disp.php?contcode=<?php echo $val["code"]?>">詳細</a></td>
            </tr>
        <?php endforeach?>
    </table>
    <br>
    ...
    <?php for($i=$now_page-5; $i<$now_page; $i++):?>
        ...
        <?php if($i <= 0):?>
            <?php echo "";?>
        <?php else:?>
            <a href="cont_post_list.php?usercode=<?php echo $user_code?>&page_id=<?php echo $i?>"><?php echo $i?></a>　
        <?php endif?>
    <?php endfor?>
